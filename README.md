...
## Guardrail Agent And Memory

`pipeline/guardrails.py` is a deterministic safety sub-agent that runs before the
LLM and again before every requested tool call. It intercepts emergencies and
cross-guest privacy requests, validates booking and retrieval arguments, and
stops repeated tool-call loops. Safety decisions do not depend on a second LLM.

The guardrail keeps append-only memory in `logs/guardrail-memory.jsonl` by
default. Records include policy version, session ID, decision reason, allowed
argument names, and state changes such as language switches or transfer actions.
Raw caller text, guest names, and contact details are omitted and represented by
...
The memory is an audit aid, not an authorization database. A production booking
integration still needs authenticated sessions, transactional confirmation
state, idempotency, retention controls, and centralized durable storage.
