...
class Agent:
    ...
    def __init__(self, provider: Provider, guardrail: GuardrailAgent | None = None):
    ...
    def respond(self, user_text: str, trace: TurnTrace | None = None) -> tuple[str, str | None]:
        ...
        """
        trace = trace or TurnTrace(session_id=self.session_id)
        ...
        input_decision = self.guardrail.evaluate_input(user_text, trace.session_id)
        trace.event("caller.transcript", text=user_text)
        trace.event(
            "guardrail.input_evaluated",
        ...
        if not input_decision.allowed:
            ...
            return input_decision.response, input_decision.action
        ...
        while True:
            ...
            for tc in msg.tool_calls:
                ...
                with trace.span("tools", tool=tc.function.name):
                    ...
                    trace.event(
                        "guardrail.tool_evaluated",
                        tool=tc.function.name,
                        allowed=tool_decision.allowed,
                        category=tool_decision.category,
                        reason=tool_decision.reason,
                    )
                    if not tool_decision.allowed:
                        result = {
                            "result": tool_decision.response,
                            "action": tool_decision.action,
                        }
                    elif tc.function.name == "set_language":
                ...
                if tool_decision.allowed:
                    self.guardrail.remember_tool_result(
                        trace.session_id,
                        tc.function.name,
                        result,
                    )
                ...
                if not tool_decision.allowed and tool_decision.category == "tool_budget":
                    trace.event("guardrail.tool_loop_stopped", action=tool_decision.action)
                    self.messages.append({
                        "role": "assistant",
                        "content": tool_decision.response,
                    })
                    return tool_decision.response, tool_decision.action
